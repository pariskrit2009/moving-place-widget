...
export function useAutocompleteSuggestions(
  ...
  // raw suggestions from the API (not cleared when input is empty)
  const [rawSuggestions, setRawSuggestions] = useState<
    google.maps.places.AutocompleteSuggestion[]
  >([]);

  // derived: empty input → empty suggestions
  ...
  const fetchSuggestions = (
    ...
    suggestions: typeof google.maps.places.AutocompleteSuggestion,
  ) => {
    setIsLoading(true);
    suggestions.fetchAutocompleteSuggestions(request).then((res) => {
      setRawSuggestions(res.suggestions);
      setIsLoading(false);
    });
  };
